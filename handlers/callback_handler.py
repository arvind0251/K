...
from database.models import get_user, update_user, get_all_countries, get_all_services
...
Total Recharged: ₹{user['total_recharged']}"""
        ...
        buttons = [[InlineKeyboardButton(c['name'], callback_data=f"otp_country_{c['code']}")] for c in countries]
    ...
    elif data == "admin_prices" and chat_id == ADMIN_ID:
        services = get_all_services()
        if not services:
